...
@Component({
  selector: 'app-checkouts-diarios',
  templateUrl: './checkouts-diarios.component.html',
  styleUrls: ['./checkouts-diarios.component.css','./checkouts-diarios2.component.css','./checkouts-diarios3.component.css']
})
export class CheckoutsDiariosComponent {
  ...
  // Mapa de terceirizados (faxina) id -> nome
  private terceirizadosMap: Map<number, string> = new Map();
  ...
  ngOnInit(): void {
    // Carregar terceirizados para mapear nomes
    this.userService.getUsersByRole('terceirizado').subscribe({
      next: (users) => {
        users.forEach(u => {
          if (u.id !== undefined && u.first_name) {
            this.terceirizadosMap.set(Number(u.id), String(u.first_name));
          }
        });
      },
      error: () => {}
  ...
  carregarReservasPorPeriodo(): void {
    ...
    // Para checkouts, usamos o endpoint de faxinas (fim dentro do período)
    this.reservasAirbnbService.getFaxinasPorPeriodo(this.dataInicio, this.dataFim)
      .subscribe({
        next: (reservas) => {
          const tratadas = this.tratarReservas(reservas);
          // Ordena por data de checkout crescente
          this.todasReservas = this.todasReservas.slice().sort((a, b) => {
            const ea = this.toIsoDate(a.end_data || '');
            const eb = this.toIsoDate(b.end_data || '');
            return ea.localeCompare(eb);
          });

          this.reservasFiltradas = [...tratadas];
  ...
  // Obtém o rótulo da pessoa da limpeza respeitando possíveis nomes retornados pelo backend
  getFaxinaLabel(reserva: ReservaAirbnb): string {
    const anyReserva: any = reserva as any;
    const nome = anyReserva.faxina_userName || anyReserva.faxina_nome;
    const id = reserva.faxina_userId;
    if (nome) return String(nome);
    if (id) {
      const mapped = this.terceirizadosMap.get(Number(id));
      return mapped ? mapped : String(id);
    }
    return '-';
  }
